...
> **按子周期推进**：**3a BPMN 模型设计器 ✅**（`feature/workflow-bpmn`）/ **3b 动态主数据 ✅**（`feature/dynamic-master-data`）/ 3c 工序内容编制 + 工艺查询（待开始，下一支）。旧扁平工艺 BOM（`/technology/bom`，菜单152）按 1c-2 既定**不实现**。
>
...
- ☐ 其余周期：Cycle 3 剩余（**3c 工序内容编制+工艺查询**）/ Cycle 4（性能/打磨/测试）。**Cycle 2 全部完成（2a/2b-1/2b-2/2c）；Cycle 3 已完成 3a/3b。**
- **下一步**：3b 浏览器 :4200 冒烟确认（需后端 9090 + dev DB 已有菜单 105/106）；启动 **3c 工序内容编制 + 工艺查询**（`/technology/process-content/*`，参考 mes-new 2f，从 `develop` 切）。
