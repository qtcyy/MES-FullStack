...
> **按子周期推进**：**3a BPMN 模型设计器 ✅**（`feature/workflow-bpmn`）/ 3b 动态主数据（Layer1 表配置 + Layer2 动态数据维护，待开始）/ 3c 工序内容编制 + 工艺查询（待开始）。旧扁平工艺 BOM（`/technology/bom`，菜单152）按 1c-2 既定**不实现**。
>
...
- ☐ 其余周期：Cycle 3 剩余（**3b 动态主数据** / **3c 工序内容编制+工艺查询**）/ Cycle 4（性能/打磨/测试）。**Cycle 2 全部完成（2a/2b-1/2b-2/2c）。**
- **下一步**：3a 浏览器 :4200 冒烟确认（需后端 9090 + DB 跑 `workflow-config-tables.sql`+`workflow-flow-config.sql`）；`feature/workflow-bpmn` `--no-ff` 合 `develop`；启动 **3b 动态主数据**（`sp_table_manager` 表配置 Layer1 + 通用动态数据维护 Layer2，后端已加固，参考 mes-new 2j/2j-2）。
