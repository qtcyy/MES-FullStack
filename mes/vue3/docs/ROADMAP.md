...
> **按子周期推进**：**2a 库存 ✅**（`feature/inventory`）/ 2b 剩余基础数据（拆 **2b-1 设备/零部件/设备编组 ✅** `feature/basedata-device` + **2b-2 仓库库位/加工单元 CRUD ✅** `feature/basedata-warehouse-unit`，**Cycle 2 基础数据收尾**）/ **2c 组织·班组 ✅**（`feature/team-organization`，班组管理含成员 + 加工单元-班组关联面板，**Cycle 2 收官**）。
>
> **✅ 2c 组织·班组完成（2026-06-22，分支 `feature/team-organization`，待 `--no-ff` 合 `develop`）——Cycle 2 收官**：两块一支——**班组管理**（`/system/team` 主从：左班组 CRUD code/name/上下班时间/工作日多选/备注 + 软删，右成员维护面板内联 `DualListTransfer` + diff 保存）、**加工单元页升级主从**（`/basedata/process-unit` 就地改造为 `MasterDetailLayout`，左原 CRUD 全保留 + 右关联班组面板）。对接后端**已存在** 11 端点（班组 8 + 加工单元 teams 3），**零后端生产代码改动**。沉淀 `utils/team`（parseWorkdays/formatWorkdays/workdaysLabel/validateTeam/buildTeamPayload/teamUserToTransferItem/teamToTransferItem，**TDD 12 例**）+ `api/system/team.ts`（8 端点）+ `api/basedata/processUnit.ts`（补 teams 3 端点）+ `types/team.ts`（SpTeam/SpTeamDTO/TeamFormModel）。**复用 2b-1 沉淀的 `DualListTransfer` + utils/device 的 `diffMembers`/`excludeSelected`**（不重定义）。**关键决策（沿用 mes-new 2l）**：生产线/车间（lineId/workshopId）不做（`sp_line`/`sp_work_shop` 无数据源）；成员/班组绑定统一用**内联穿梭框 + 保存按钮 + diff**形态（对齐 device-group 姊妹模式，非弹窗）。**编码**：team page/add-or-update 走 form，delete/users-add/users-remove 走 JSON；process-unit teams/{unitId} 走 GET、teams/add、teams/remove 走 JSON。**后端审查（按 [[backend-deepseek-review-each-cycle]] 每周期必审）结论 ZERO EXPOSED BUGS、零改动**：`pageWithRelations` 经 `SpTeamMapper.xml` `WHERE t.is_deleted != '1'` 过滤软删 + userCount 关联子查询正确；delete 软删经 `@TableField("is_deleted")` 映射正确;**关键怀疑点 `/teams/{unitId}` 的空 `listByIds` 已有 `items.isEmpty()` 守卫提前返回**（避开 MyBatis-Plus 3.1.2 空 IN 隐患）；users/add 与 teams/add 均有 existence 去重；available-users 用 `is_deleted='0'`（更严,排除禁用,合理）。**菜单**：复用既有 `scripts/sql/team-management.sql`（建表 + 种子 + **菜单 107「班组员工定义」挂系统管理组**，url `/admin/sys/team/list-ui`，perm `team:add`，**需手动跑**）；urlMap +1（→`/system/team`）+ router +1（含 `name: 'system-team'` 对齐同模块命名约定）；加工单元零菜单/urlMap 改动（改造现页）。门禁全绿（typecheck0/**test293**[+12]/lint0err[5 既有 warn]/build✓）。subagent 驱动逐任务两阶段审查（基础层 + 班组页簇 + 加工单元簇逐簇审查）+ 后端独立审查。spec/plan：`docs/specs|plans/2026-06-22-cycle2c-team-organization*`。**2c-backlog（非阻塞）**：① `utils/team` 的 `validateTeam`/`teamToTransferItem` 部分未被视图消费（靠 el-form rules 校验，与 device-group 兄弟一致）② 候选池 `teamPage`/`available-users` 大 size 兜底全量（PaginationInterceptor 上限，同 1f/2a/2b-1）③ `users/add`、`teams/add` 无 `@Transactional`（低危）④ `getTeamUsers`/`/teams/{unitId}` 不过滤成员/班组软删（与 mes-new 2l 一致判定）。**人工 :4200 冒烟待确认**：需后端 9090 + DB 跑 `team-management.sql`，`admin/123` 登录 → 系统管理 → 班组员工定义（新建填上下班/工作日 → 选中 → 成员穿梭加减保存 → 编辑回填 → 软删）+ 物料管理 → 加工单元（选中 → 关联班组穿梭加减保存）。
>
...
### Cycle 3 — 工作流设计器 + 动态主数据 + 工艺深化（✅ 收官）
BPMN 模型设计器（bpmn-js，保存/发布）；动态主数据（`sp_table_manager` + item + 通用动态数据 CRUD）；工序内容完整（设备/文档/上传）+ 工艺查询页。

> **按子周期推进**：**3a BPMN 模型设计器 ✅**（`feature/workflow-bpmn`）/ **3b 动态主数据 ✅**（`feature/dynamic-master-data`）/ 3c 工序内容编制 + 工艺查询 —— **3c-1 工艺内容编制 ✅**（`feature/process-content`）/ **3c-2 工艺查询只读页 ✅**（`feature/process-query`，**Cycle 3 收官、整条业务线完结**）。旧扁平工艺 BOM（`/technology/bom`，菜单152）按 1c-2 既定**不实现**。
>
> **✅ 3c-2 工艺查询只读页完成（2026-06-23，分支 `feature/process-query` 已 `--no-ff` 合入 `develop` 并删分支；develop 超前 origin 未 push，用户自行 push）**：交付 `/technology/process-query`（菜单 116「产品工艺查询」已实证存在 dev DB，url=`/technology/process-query`、perm `process-query:list`、父15，**零 menu seed SQL**），**零后端生产代码改动**。**浏览→主从只读**：产品下拉 → **选产品即展开主从 + 自动选中产品根节点**（区别于 3c-1 的"进入"两态）→ 左 `TreeTable` BOM 树（节点名/层级 levelLabel/编制状态徽标）→ 右只读 7 Tab 查看器（主信息/工序要求/检验/注意事项/工装设备/技术文档/物料清单，全 `el-input readonly` + `MultiImageUpload disabled`，无任何按钮/写入入口）。复用 3c-1 的 4 个只读 GET（pcProducts/pcList/pcGet/pcBomItems）+ `TreeTable`/`MasterDetailLayout`/`PageContainer`/`MultiImageUpload`/`buildTreeFromList`/`parseCsvKeys`/`inspectionToBool`。**唯一新增逻辑 `levelLabel`**（0→产品/1→半成品/≥2→组件/undefined→产品，TDD 2 例 5 断言）。新增 2 视图组件（ProcessQueryDetail/ProcessQueryPage，均纯 props 入无 emit）+ router +1（meta perm）+ urlMap +1 自映射。`selectNode` 照搬 selToken 竞态守卫；`onPickProduct` 加 try/catch（useRequest.run re-throw，消除树加载失败未捕获 rejection——相对参考页的防御性改进）；文档链接 `fileUrl` 缺失时 `:disabled`（防欺骗性点击）。**后端审查（按 [[backend-deepseek-review-each-cycle]]）ZERO EXPOSED BUGS 零改动**：4 个 GET 端点逐个读码核验——`/list/{rootId}` 含根（`result.add(root)` 首行）、图片 key 单次重签不双签、物料/产品物理删无软删问题（mes-new 2f 修 12 bug + 2k curl 验证 + 3c-1 后端审查同份后端）。门禁全绿（typecheck0/**test333**[+2]/lint0err[5既有warn]/build✓，ProcessQueryPage 独立懒加载 chunk 6.66KB/gz2.60KB）。subagent 驱动逐任务两阶段审查（spec+质量）+ 独立后端审查 + opus 整体终审 **READY TO MERGE**（跨文件类型/字段逐项核对、selToken+自动选根流程、只读完整性零写入入口确认）。spec/plan/验证：`docs/specs|plans/2026-06-23-cycle3c2-process-query*` + `…cycle3c2-verify-results.md`。**3c-2-backlog**：① 遗留数据双签 latent（同 3c-1-backlog，新数据裸 key 不触发）② BOM 树 N+1 递归（层级≤5 无影响）③ `/products` 不按状态过滤（业务合理）④ statusLabel 草稿判定看 content.id、列表徽标看 contentStatus（口径不同结果一致，镜像 3c-1）。**人工 :4200 冒烟待确认**：后端 9090 + dev DB 有产品 BOM + 菜单 116，`admin/123` → 工艺管理 → 产品工艺查询：选产品→左树自动选根→右 7 Tab 只读浏览→切节点不错配。
>
> **✅ 3c-1 工艺内容编制完成（2026-06-23，分支 `feature/process-content` 已 `--no-ff` 合入 `develop` 并删分支；develop 超前 origin 未 push，用户自行 push）**：交付 `/technology/process-content`（菜单 115 已存在 dev DB，perm `process-content:list`），**零后端生产代码改动**。**浏览→主从编辑**：产品选择器 → 左 `TreeTable` BOM 树（`pcList` 重建 + 编制状态徽标）→ 右 `el-tabs` **7 Tab 编辑器**（主信息 mainInfo/content/工序图、工序要求、检验 inspectionRequired Switch+检验图、注意事项、工装设备子表、技术文档 PDF、物料清单只读）。**状态机** draft 可编辑/completed 全只读（隐藏增删上传+禁用输入，Tab 仍可切）；**contentId 引导**（无 content 仅主信息可填，保存拿 contentId 后设备/文档 Tab 解锁）。**新原语 `MultiImageUpload.vue`**（多图 key 列表↔url 展示，uploadFn 注入返回 `{key,url}`，对齐既有 ImageUpload 的 http-request 模式）。沉淀 `utils/processContent.ts`（parseCsvKeys/joinKeys/inspectionToBool/canEditContent/validateContent/buildContentPayload/buildEquipmentPayload/buildTreeFromList，**TDD 12 例**）+ `api/technology/processContent.ts`（读4/写6 JSON/上传2）+ `types/technology.ts` 扩展 + 3 视图组件。**关键契约**：写端点全 JSON、图片存逗号 key 列表后端重签、inspectionRequired '1'/'0' 不发 boolean、**更新不发 status**（后端管理）、设备/文档需 contentId。**架构决策**：内容 save/complete 走父 emit（改树徽标+saving spinner）；设备/文档**子资源 CRUD 编辑器内直调 API + emit reload**（成功才关弹窗、失败保留输入）；selectNode/reloadNode 共用 selToken token 守卫防快速切节点乱序；**reloadNode 静默刷新**（不卸载编辑器、保存后保留当前 Tab）。**后端审查（按 [[backend-deepseek-review-each-cycle]]）ZERO EXPOSED BUGS 零改动**：save 状态机（创建 draft/同 bomId 复用 id/更新保 status/拒改 completed/不信任客户端 status）、complete、validateEditableParent（设备/文档 save 前校验父存在且未完成）、图片 key 重签管线（upload 返裸 key、get 单次重签不双签）、document/delete 清 MinIO 容错——全部正确（mes-new 2f 修 12 bug + 2k curl 验证同份后端）。**菜单 115 已存在 dev DB（零 SQL）**；router +1（含 meta）+ urlMap +1 自映射。门禁全绿（typecheck0/**test331**[+12]/lint0err[5既有warn]/build✓，独立懒加载 chunk）。subagent 驱动逐簇两阶段审查（基础层/编辑器/主从页+接线/后端审查）+ opus 整体终审 **READY TO MERGE**（12 端点契约逐条闭环、状态机三重门控、token 守卫闭环）。spec/plan/验证：`docs/specs|plans/2026-06-22-cycle3c1-process-content*` + `…cycle3c1-verify-results.md`。**3c-1-backlog**：① 图片即时上传孤儿（用户上传不保存/删图不调后端删对象,即时上传固有取舍）② 后端遗留数据双签 latent（新数据裸 key 不触发）③ EquipmentFormModel 类型三处手写重复（轻微 DRY）。**人工 :4200 冒烟待确认**：后端 9090 + dev DB 有产品 BOM + 菜单 115,`admin/123` → 工艺内容编制：选产品→树选节点→主信息保存→子 Tab 解锁→多图/PDF 上传→设备增删→完成编制只读→切节点不错配+保存留 Tab。
>
> **✅ 3b 动态主数据完成（2026-06-22，分支 `feature/dynamic-master-data` 已 `--no-ff` 合入 `develop` 并删分支；develop 超前 origin 未 push，用户自行 push）**：一支交付两页，**零后端生产代码改动**——**Layer1 动态表配置** `/basedata/manager`（菜单 105，列表 + 表头/字段明细编辑大弹窗：增删字段行/上下移/必填开关）+ **Layer2 动态数据维护** `/basedata/manager-item`（菜单 106，`MasterDetailLayout` 主从:左选已登记表→右按字段元数据动态出列+动态行表单 CRUD）。对接后端**已存在** 7 端点（Layer1 page/item-by-tableNameId/add-or-update[JSON]/delete + Layer2 common/page、common/add-or-update[form 平铺 jsTableName/jsTableNameId/id?/字段值]、common/delete）。沉淀 `utils/manager.ts`（parseMustFill/validateManagerForm/buildUpsertPayload/moveRow，TDD 11 例）+ `utils/managerData.ts`（buildColumns/emptyRow/validateRow/buildDataPayload，TDD 7 例）+ `types/manager.ts` + `api/basedata/{manager,managerData}.ts` + 4 视图组件。**关键契约坑（已处理）**：① `mustFill` 读容忍 Y/y/1、写回统一 "1"/"0"；② 编辑剥 item id（后端删旧重插）；③ Layer1 `add-or-update` 走 JSON、其余 6 端点走 form;④ **Layer1 upsert 固定 `isDeleted="0"`**（后端 `sp_table_manager.is_deleted` NOT NULL 无默认 + 不自动填充,漏传则 INSERT 失败、非 "0" 让 Layer2 白名单拒绝该表，对齐 mes-new 参考）;⑤ **Layer2 删除必传 `tableNameId`**（CommonDto 字段名,后端 `commonDelete` 经 `assertTableWhitelisted(tableName,tableNameId)` 校验,漏传则删除必失败）。**两处异步竞态守卫**：ManagerForm 编辑回填 loadToken、ManagerDataPage selectTable(selectToken)+loadRows(dataToken)，快速切表/切行乱序返回不错配。**菜单零新增**——105/106 已实证存在 dev DB（url `/basedata/manager/list-ui`、`/basedata/manager/item/list-ui`，perm `manager:add`），仅 urlMap +2 + router +2。**后端审查（按 [[backend-deepseek-review-each-cycle]]）ZERO EXPOSED BUGS 零改动**：Layer1 `saveOrUpdateWithItems` 事务/删旧重插/返回 id 在位、Layer2 三层加固（表名白名单+is_deleted='0'校验/列名正则 `^[A-Za-z0-9_]+$`/值 `#{}` 参数化/is_deleted putIfAbsent）全部正确、List<Map> 经 PaginationInterceptor 真分页（mes-new 2j/2j-2/2k 已 curl 验证同份后端）。门禁全绿（typecheck0/**test319**[+18]/lint0err[5既有warn]/build✓，两页独立懒加载 chunk）。subagent 驱动逐簇两阶段审查（基础层/Layer1/Layer2/接线）+ 独立后端审查 + opus 整体终审（终审抓到并修复 1 个 Critical:Layer2 删除漏传 tableNameId）。spec/plan/验证：`docs/specs|plans/2026-06-22-cycle3b-dynamic-master-data*` + `…cycle3b-verify-results.md`。**3b-backlog**:① 全局错误返回依赖 x-requested-with 头(axios 已带,既有全局行为)② 字段类型感知=全文本框(YAGNI)③ 软删过滤不做(通用动态表不保证有 is_deleted 列)。**人工 :4200 冒烟待确认**（需后端 9090 + dev DB 已有菜单 105/106;`admin/123` → 基础数据 → 配置平台新建/编辑/删动态表 + 维护页选表动态增删改翻页 + 快速切表验证列/数据不错配）。
>
> **✅ 3a BPMN 模型设计器完成（2026-06-22，分支 `feature/workflow-bpmn`，待 `--no-ff` 合 `develop`）**：单页 `/workflow/model`（菜单 192，完成工作流模块最后一块）——列表（DataTable 搜索/分页 + 新建/设计/发布/删除 + 状态徽标）+ **全屏 `el-dialog` bpmn-js 设计器**（左 `BpmnDesigner.vue` vanilla bpmn-js 包装 + 右 `PropertiesPanel.vue` 节点办理人配置 + 顶部检查定义/保存）+ 新建弹窗 + 发布弹窗。对接后端**已存在** 5 端点（page/get/save/delete/publish）**零后端改动**。**新依赖 `bpmn-js@18.18.0` + `diagram-js-minimap@5.3.0`**（vanilla JS，Vite8/rolldown **开箱即用零 vite.config 改动**，route-level 懒加载落入 `/workflow/model` 独立 chunk 586KB/gz169KB 不污染首屏）。沉淀 `utils/bpmn.ts`（initialBpmnXml[XML转义]/validateSummary/errorTaskIds/buildAssigneeProps，**TDD 8 例**，近乎逐字移植 mes-new 2m bpmnUtils）+ `utils/flowableModdle.ts`（moddle 扩展让导出 XML 真带 `flowable:` 命名空间）+ `assets/styles/bpmn-theme.css`（设计器外壳主题，**适配 Element Plus `--el-*` 变量** + `.bpmn-error` 红色脉冲）+ `api/workflow/model.ts`（5 端点）+ `types/workflow.ts` 扩展 + 手写 `types/bpmn-js.d.ts`。**React→Vue 移植**：`useImperativeHandle`→`defineExpose`（getXML/getSummary/updateSelected/markErrors/clearErrors），bpmn-js Modeler 实例用普通 `let`（不进 ref/reactive 避免 Vue 代理），`onMounted` 创建/`onBeforeUnmount` destroy（仿 1f WarehouseScene）。**校验薄取数+纯函数分离**：`getSummary`(读 elementRegistry)+ `validateSummary`(纯函数)，保持 vitest node 环境不引 jsdom。**关键决策**：CustomRenderer 自绘节点本周期不做（`bpmnTheme.ts` 随之不移植，避免死代码）；设计器宿主全屏弹窗 `destroy-on-close` 每次重挂载；BpmnDesignerDialog 用 `watch` 第三参 `onCleanup` 做取数 ignore 守卫。**后端审查（按 [[backend-deepseek-review-each-cycle]] 每周期必审）结论 ZERO EXPOSED BUGS 零改动**：save 唯一性 `.ne(id)` 条件式排除自身、有 id 仅更新 name/key/xml 保留 status/version（发布不可逆）、publish upsert 定义 `def.id=model.id`（saveOrUpdate 预设 id 走 insert 不重生成）、delete `@Transactional` 级联清定义+事件规则、page 经 PaginationInterceptor 真分页 + LIKE 参数化无注入、bpmnXml longtext 经 JSON `#{}` 转义往返安全（mes-new 2n 已 curl 验证同份后端）。**菜单 192 已种**（`workflow-flow-config.sql`，url `/workflow/model/list-ui`，perm `workflow:model:list`，**需手动跑**，1h 已要求）；urlMap +1（替换 1h 占位注释）+ router +1（name `workflow-model`）。门禁全绿（typecheck0/**test301**[+8]/lint0err[5既有warn]/build✓）。subagent 驱动逐簇两阶段审查（基础层/设计器组件/编排簇/装配簇）+ 独立后端审查 + opus 整体终审。spec/plan：`docs/specs|plans/2026-06-22-cycle3a-workflow-bpmn*`。**3a-backlog（非阻塞）**：① CustomRenderer 自绘节点语义配色（视觉深化，待落地再移植 bpmnTheme.ts）② getXML 已加 `?? ''` 兜底；getSummary 把「仅开始事件」空模型算 1 孤立节点（提示略噪不阻断）③ 后端 publish/save 未校验 category/name 非空（靠前端兜底，LATENT）④ 权限守卫粒度（新建按钮用 `:list` perm，后端仅粗粒度）⑤ 后端运行时（实例/任务/事件触发）留将来。**人工 :4200 冒烟待确认**：后端 9090 + DB 跑 `workflow-config-tables.sql`+`workflow-flow-config.sql`，`admin/123` 登录 → 流程配置工具 → 流程模型设计：新建→设计（拖用户任务/连线/配办理人）→检查定义（漏结束/办理人→红色脉冲+toast）→保存→发布到分类→流程定义页出现该定义→删除级联。
...
| 班组管理（成员 + 加工单元关联） | `/admin/sys/team/*` `/basedata/process-unit/teams/*` | C2·2c | ✅ |
...
| 动态主数据配置 + 维护 | `/basedata/manager/*` `/basedata/common/*` | C3·3b | ✅ |
...
| 工序内容（设备/文档/上传） | `/technology/process-content/*` | C3·3c-1 | ✅ |
...
| BPMN 模型设计器（bpmn-js） | `/workflow/model/*` | C3·3a | ✅ |
...
- ✅ **子周期 2c 组织·班组完成（2026-06-22，分支 `feature/team-organization` → 待 `--no-ff` 合 `develop`）——Cycle 2 收官**：班组管理主从（CRUD 含上下班/工作日 + 成员内联穿梭 diff 保存）+ 加工单元页升级主从（右侧关联班组面板）+ `utils/team` TDD 12 例。**零后端改动**（后端独立审查 ZERO EXPOSED BUGS，含 `/teams/{unitId}` 空 `listByIds` 已守卫）；复用 2b-1 `DualListTransfer` + utils/device `diffMembers`。菜单复用 `team-management.sql`（107，需手动跑）；urlMap +1、router +1。门禁全绿（typecheck0/**test293**/lint0err/build✓）。详见 §8 Cycle 2 段。
- ✅ **子周期 3a BPMN 模型设计器完成（2026-06-22，分支 `feature/workflow-bpmn` → 待 `--no-ff` 合 `develop`）**：单页 `/workflow/model`（菜单 192）bpmn-js 全屏设计器（建模/办理人配置/校验高亮/发布派生定义）+ `utils/bpmn` TDD 8 例。新依赖 bpmn-js@18+minimap（Vite8/rolldown 开箱即用，route 懒加载独立 chunk）。零后端改动（后端独立审查 ZERO EXPOSED BUGS）。门禁全绿（typecheck0/**test301**/lint0err/build✓）。详见 §8 Cycle 3 段。
- ✅ **子周期 3b 动态主数据完成（2026-06-22，分支 `feature/dynamic-master-data` 已 `--no-ff` 合入 `develop` 并删分支）**：一支交付 Layer1 动态表配置页（`/basedata/manager`，列表+大弹窗）+ Layer2 动态数据维护页（`/basedata/manager-item`，主从 选表→动态列+动态行 CRUD），零后端改动。沉淀 `utils/manager`(TDD11)+`utils/managerData`(TDD7)。终审抓修 1 Critical(Layer2 删除漏传 tableNameId)。门禁全绿(typecheck0/**test319**/lint0err/build✓)。详见 §8 Cycle 3 段。
- ✅ **子周期 3c-1 工艺内容编制完成（2026-06-23，分支 `feature/process-content` 已 `--no-ff` 合入 `develop` 并删分支）**：交付 `/technology/process-content`（菜单115，主从+7Tab 编辑器+状态机+contentId 引导），零后端改动。新原语 MultiImageUpload(多图)。沉淀 `utils/processContent`(TDD12)。内容交父 emit、子资源直调 API+reload、token 守卫+静默刷新。后端审查 ZERO EXPOSED BUGS。门禁全绿(typecheck0/**test331**/lint0err/build✓)。详见 §8 Cycle 3 段。
- ☐ 其余周期：Cycle 3 剩余（**3c-2 工艺查询只读页**，菜单116）/ Cycle 4（性能/打磨/测试）。**Cycle 2 全部完成（2a/2b-1/2b-2/2c）；Cycle 3 已完成 3a/3b/3c-1。**
- **下一步**：3c-1 浏览器 :4200 冒烟确认（需后端 9090 + dev DB 有产品 BOM + 菜单 115）；启动 **3c-2 工艺查询只读页**（`/technology/process-query`，菜单116，产品→BOM树→只读 Tabs+图片，复用 3c-1 只读 GET 端点，参考 mes-new 2k，从 `develop` 切）。
- **（历史）下一步**：2b-2 浏览器 :4200 冒烟确认（需跑仓库/加工单元建表脚本 + `warehouse-unit-menu-seed.sql`）。
