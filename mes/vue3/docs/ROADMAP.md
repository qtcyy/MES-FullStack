...
> **按子周期推进**：**3a BPMN 模型设计器 ✅**（`feature/workflow-bpmn`）/ **3b 动态主数据 ✅**（`feature/dynamic-master-data`）/ 3c 工序内容编制 + 工艺查询 —— **3c-1 工艺内容编制 ✅**（`feature/process-content`）/ 3c-2 工艺查询只读页（菜单 116，待开始，下一支）。旧扁平工艺 BOM（`/technology/bom`，菜单152）按 1c-2 既定**不实现**。
>
