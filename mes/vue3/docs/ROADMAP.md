...
> **按子周期推进**：**2a 库存 ✅**（`feature/inventory`）/ 2b 剩余基础数据（拆 **2b-1 设备/零部件/设备编组 ✅** `feature/basedata-device` + **2b-2 仓库库位/加工单元 CRUD**，待开始）/ 2c 组织·班组（成员，含加工单元-班组关联面板，待开始）。
>
...
- ☐ 其余周期：Cycle 2 剩余（2b-2 仓库库位/加工单元 CRUD / 2c 组织·班组）/ Cycle 3（工作流设计器 + 动态主数据 + 工艺深化）/ Cycle 4（性能/打磨/测试）。
