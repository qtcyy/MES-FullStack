...
> **按子周期推进**：**2a 库存 ✅**（`feature/inventory`）/ 2b 剩余基础数据（设备/设备编组/加工单元/仓库库位/零部件，待开始）/ 2c 组织·班组（成员，待开始）。
>
...
- ☐ 其余周期：Cycle 2 剩余（2b 剩余基础数据 / 2c 组织·班组）/ Cycle 3（工作流设计器 + 动态主数据 + 工艺深化）/ Cycle 4（性能/打磨/测试）。
- **下一步**：2a 浏览器 :4200 冒烟确认（需跑两个库存 seed SQL）；`feature/inventory` `--no-ff` 合 `develop`；启动 2b（剩余基础数据）。
